...
general.getCates = async () => {
    let getCate = `SELECT categories.*, COUNT(products.product_id) AS category_count
                FROM categories
                LEFT JOIN products
    ...
    return new Promise((resolve, reject) => {
        console.log('🔍 Running getCates query...')  // ✅ LOG 1
        
        db.query(getCate, async (err, cates) => {
            console.log('📊 Query result:', { error: err, catesCount: cates?.length })  // ✅ LOG 2
            ...
            try {
                ...
                cates.forEach((cate) => {
                    promises.push(
                        general.getBestSellerProductsOfCates(Number(cate.category_id), 8)
                            .then((bestSellerProducts) => {
                                cate.bestSellerProductsOfCates = bestSellerProducts
                            })
                            .catch(err => console.error('❌ Error getting best sellers:', err))
                    )
                })

                await Promise.all(promises)
                console.log('✅ getCates resolved with', cates.length, 'categories')  // ✅ LOG 3
                resolve(cates)
            } catch (error) {
